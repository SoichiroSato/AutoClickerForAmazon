...
def main():  
    ...
    while True:
        try:
            ...
            before2Minutes = datetime.combine(date.today(), start) - timedelta(minutes=2)
            loginTime = datetime.combine(date.today(), before2Minutes.time()) #購入予定時刻の2分前
    ...
    #ログイン処理実行時刻まで待機
    TimeUtility.Sleep(TimeUtility.FindTheTimeDifference(loginTime,ntpClient))
    ...
    #購入処理実行時刻まで待機
    TimeUtility.Sleep(TimeUtility.FindTheTimeDifference(purchaseTime,ntpClient))
